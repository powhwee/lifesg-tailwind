...
import { allLeafPaths, firstLeafOf, leafByPath } from "@/components/overlays/registry";
...
export default async function OverlaysPage({
  ...
  if (slug.length === 1) {
    ...
    if (!direct) {
      ...
      if (target) redirect(`/overlays/${target}`);
      notFound();
    }
  }
  ...
  return (
    <div className="flex flex-col min-h-screen">
      ...
      <div className="grid grid-cols-2 flex-1">
        <section aria-label="ours" className="p-8 border-r border-border" data-testid="overlays-ours">
          <div className="text-xs uppercase tracking-wide text-muted-foreground mb-4">shadcn + L3 (ours)</div>
          <Ours />
        </section>
        <section aria-label="LifeSG reference" className="p-8" data-testid="overlays-lifesg">
          <div className="text-xs uppercase tracking-wide text-muted-foreground mb-4">LifeSG (reference)</div>
          <LifeSGProvider><LifeSG /></LifeSGProvider>
        </section>
      </div>
    </div>
  );
