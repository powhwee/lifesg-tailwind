...
import { tree } from "@/components/overlays/registry";
  ...
  return (
    <div className="flex min-h-screen">
      <aside className="w-72 shrink-0 border-r border-border bg-[var(--lifesg-bg-strong)] p-6">
        <Link href="/" className="block text-xs text-muted-foreground hover:text-[var(--lifesg-text-hover)] mb-3">
          ← Pilot home
        </Link>
        ...
        <nav className="flex flex-col gap-0.5 text-sm">
          {tree.map((node) => {
            if (node.kind === "leaf") {
              return (
                <Link key={node.slug} href={`/overlays/${node.slug}`} className={linkCx}>
                  <FileIcon />
            ...
            return (
              <details key={node.slug} className="group" open>
                ...
                <div className="flex flex-col gap-0.5 ml-6 mt-0.5">
                  {node.children.map((child) => (
                    <Link key={child.slug} href={`/overlays/${node.slug}/${child.slug}`} className={linkCx}>
                      <FileIcon />
