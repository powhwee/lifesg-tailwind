...
import { tree } from "@/components/selection-and-input/registry";
...
export default function SelectionAndInputLayout({ children }: { children: React.ReactNode }) {
  return (
      ...
      <aside className="w-72 shrink-0 border-r border-border bg-lifesg-bg-strong p-6">
        ...
        <nav className="flex flex-col gap-0.5 text-sm">
          {tree.map((node) => {
            if (node.kind === "leaf") {
              return (
                <Link key={node.slug} href={`/selection-and-input/${node.slug}`} className={linkCx}>
                  <FileIcon />
