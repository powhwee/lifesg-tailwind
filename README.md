...
The verification system went through significant hardening — see [Annex A: How the verification system evolved](#annex-a-how-the-verification-system-evolved) for the full history of gaps identified and addressed.
...
## Annex A: How the verification system evolved
...
| **Tailwind 4 `@theme inline` namespace mismatches silently dropped utilities.** Two distinct cases caught: (a) `text-field-label` (font-size) was stripped by `tailwind-merge` when paired with `text-field-text-label` (color) — same prefix, no classGroup registration — so label rendered at inherited 16px instead of the token's 18px. (b) `--card-shadow` (and drawer/modal/popover-shadow) registered as `--color-X-shadow` instead of `--shadow-X-shadow` — Tailwind 4 generates `shadow-X` utilities from `--shadow-X` only, so the class was present in the DOM but never produced a CSS rule. Cards shipped to production with no shadow. | Extended `tailwind-merge`'s `font-size` classGroup with every custom `--text-*` token; renamed all shadow tokens to `--shadow-X`. Added two memory entries (`[[tailwind-merge-font-size]]`, `[[tailwind4-theme-namespaces]]`) to prevent recurrence. Workflow lesson: after token edits, eyeball the rendered DOM, not just measure-* counts. The shadow bug was invisible to all 4 measure-* suites because they probe heights/widths, not box-shadow. |
| **Re-baselining was a silent regression vector.** `screenshot-all` and `playwright --update-snapshots` were run mechanically without visual review, then committed. The card-shadow bug above would have been blessed as the new baseline if the user hadn't spotted it visually. | Codified "re-baselining is a review step, not mechanical" in `[[visual-parity-workflow]]` memory. Mandatory sequence: re-snap → visually review the most impactful PNGs → investigate any user-spotted "regression" (often actual bugs) → commit only after sign-off → deploy. |
| **Visual parity drifted into pixel-chasing without a principle.** Phase 1 fixes started accumulating per-component patches (`my-6` on breadcrumb, etc.) that fought each other across the design system. Some "fixes" forced ours toward LifeSG outliers (e.g. a 656px-wide inline calendar nobody else ships). No documented rule for "match this vs keep our own." | Established "Match the language, not the layout" as the project's parity principle: components ship opinionated LifeSG-like defaults via L3 tokens, overridable at per-call / global / scoped. Calendar stays at 336px under "LifeSG outlier rejected"; everything else tunes to LifeSG when reasonable. Doc at `docs/parity-principle.md`, memory at `[[parity-principle]]`. Allowlists now have explicit category prefixes (NOISE / OUTLIER-REJECTED / DEMO-STATE / STRUCTURAL) so future maintainers don't re-chase documented divergences. |
